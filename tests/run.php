...
$provider = new LdapProvider(
    $directory,
    fn($subject) => $subject === 'immutable-uuid' ? '42' : null,
    fn($id) => $id === '42' ? 'immutable-uuid' : null,
    $accounts,
);
check(
    $provider->authenticate(new PasswordCredentials('alice', '')) === null
        && $directory->calls === 0,
    'anonymous bind risk',
);
check(
...
check(
    $provider->authenticate(new PasswordCredentials('alice', 'test-password'))?->getIdentifier()
        === '42',
    'linked account',
);
...
try {
    foreach (
        [
            ['url' => 'http://directory'],
            ['url' => 'ldap://user:secret@directory'],
            ['usernameAttribute' => 'uid)(objectClass=*'],
            ['timeout' => 0],
            ['bindPassword' => ''],
        ] as $override
    ) {
