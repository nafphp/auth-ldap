...
{
    public function __construct(private string $url, private string $baseDn, private string $bindDn, #[\SensitiveParameter] private string $bindPassword, private string $caFile, private string $usernameAttribute = 'uid', private string $subjectAttribute = 'entryUUID', private string $allowedFilter = '(objectClass=inetOrgPerson)', private int $timeout = 5)
    {
        $parts = parse_url($url);
        if (!$parts || !in_array($parts['scheme'] ?? '', ['ldap','ldaps'], true) || empty($parts['host']) || isset($parts['user']) || isset($parts['pass']) || isset($parts['query']) || isset($parts['fragment']) || !in_array($parts['path'] ?? '', ['','/'], true)) {
            throw new \InvalidArgumentException('Use one explicit ldap:// or ldaps:// server URL.');
        }
        if ($baseDn === '' || $bindDn === '' || $bindPassword === '') {
            throw new \InvalidArgumentException('LDAP requires a search base and non-anonymous service credentials.');
        }
    ...
    {
        if ($username === '' || $password === '' || strlen($username) > 254 || strlen($password) > 1024) {
            return null;
        }
    ...
    private function connect(): Connection
    {
        ...
        foreach ([LDAP_OPT_X_TLS_REQUIRE_CERT => LDAP_OPT_X_TLS_DEMAND,LDAP_OPT_X_TLS_CACERTFILE => $this->caFile] as $option => $value) {
        ...
        try {
            foreach ([LDAP_OPT_PROTOCOL_VERSION => 3,LDAP_OPT_REFERRALS => 0,LDAP_OPT_NETWORK_TIMEOUT => $this->timeout,LDAP_OPT_TIMELIMIT => $this->timeout] as $option => $value) {
    ...
    {
        $filter = '(&'.$this->allowedFilter.'('.$attribute.'='.ldap_escape($value, '', LDAP_ESCAPE_FILTER).'))';
        $result = @ldap_search($connection, $this->baseDn, $filter, [$this->subjectAttribute], 0, 2, $this->timeout);
        if (!$result) {
